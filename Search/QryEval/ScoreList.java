...
public class ScoreList {
    ...
    public boolean containsDocument(String docID){
        for(ScoreListEntry entry: scores){
            if(entry.externalId.equals(docID))
                return true;
    ...
    public void remove(String docID) {
        int internalID=-1;
        try {
            internalID = Idx.getInternalDocid(docID);
        }catch (Exception c){
            System.out.println("\n couldnt fetch internal ID while removing");

        }
        for(int i=0;i<scores.size();i++){
            if(scores.get(i).docid==internalID)
            {
                scores.remove(i);
            return;
            }
    ...
    public double get_score_for_document(String DocID){
        for(ScoreListEntry entry: scores){
            if(entry.externalId.equals(DocID))
                return entry.score;
