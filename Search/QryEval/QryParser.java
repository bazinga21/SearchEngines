...
public class QryParser {
  ...
  private static Qry createOperator (String operatorName) {
    ...
    //  #near/n and #window/n.
    if(operatorNameLowerCase.startsWith("#window")) {
      operatorDistance = Integer.parseInt(operatorNameLowerCase.substring(8));
      operatorName = operatorNameLowerCase.substring(0, 7);
      operatorNameLowerCase=operatorName;
    }
    if(operatorNameLowerCase.startsWith("#near")) {
      operatorDistance = Integer.parseInt(operatorNameLowerCase.substring(6));
      operatorName = operatorNameLowerCase.substring(0, 5);
      operatorNameLowerCase=operatorName;
    }
  ...
  private static Qry parseString (String queryString)
    ...
    while (queryString.length() > 0) {
     ...
     if(!Character.isAlphabetic(queryString.charAt(0))&&Character.isDigit(queryString.charAt(0))) {
        int space = queryString.indexOf(" ");
        weight = Double.parseDouble(queryString.substring(0, space));
        queryString = queryString.substring(space).trim();
      }
      ...
      for (int i=0; i<qargs.length; i++) {
        if(queryTree instanceof QrySopWSUM) {
          ((QrySopWSUM) queryTree).weight.add(weight);
        } else if(queryTree instanceof QrySopWAND) {
          ((QrySopWAND) queryTree).weight.add(weight);
        }
